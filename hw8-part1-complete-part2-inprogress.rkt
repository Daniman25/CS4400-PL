...
(define-type BRANG
  ...
  [Bind (Listof Symbol) (Listof BRANG)] ;; ask at OH
  [Bind* (Listof Symbol) (Listof BRANG)])
...
(define (parse-sexpr sexpr)
  (match sexpr
    ...
    [(cons (or 'bind 'bind*) more) ; match on either symbol
     (match sexpr
       [(list binder (list (symbol: names) ...) bodys ...)
      ;; we can now look at (eq? binder 'bind) for how we got here
        (if (eq? binder 'bind)
            (Bind (map parse-sexpr names) (map parse-sexpr bodys)) ;; reg br
            (Bind* (map parse-sexpr names) (map parse-sexpr bodys)))])] ;; * br
    [(cons 'call more)
     (match sexpr
    ...
    [(Call fun-expr arg-exprs)
     ...
     (if (= 1 (length arg-exprs))
       ...
       (sub (Call (Call fun-expr (list (first arg-exprs)))
                  (rest arg-exprs))))]
    [(Bind bound-ids bound-bodies) ...] ;; TODO
    [(Bind* bound-ids bound-bodies) ...])) ;; TODO
