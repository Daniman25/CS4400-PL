...
(define (parse-sexpr sexpr)
  (match sexpr
    ...
    ;; NOTE DO WE NEED TO PEEL OUT FIRST OF SYMBOL AND FIRST OF BODYS
    [(cons (or 'bind 'bind*) more) ; match on either symbol
     (match sexpr
       [(list binder (list (list (symbol: names) (sexpr: bodies)) ...) brang)
        ;; we can now look at (eq? binder 'bind) for how we got here
        (if (eq? binder 'bind)
            (Bind names (map parse-sexpr bodies) (parse-sexpr brang)) ;; reg br
            (Bind* names (map parse-sexpr bodies) (parse-sexpr brang)))])] ;; * br
    [(cons 'call more)
     (match sexpr
    ...
    [(Call fun-expr arg-exprs)
     ...
     (if (= 1 (length arg-exprs))
       ...
       (sub (Call (Call fun-expr (list (first arg-exprs)))
                  (rest arg-exprs))))]
    ;; Since multi arg funcs are allowed shouldn't need recursion basically j do what we do in the with branch b with multiple arguments type shit
    ;;note bound-ids and bound-bodies are already lists so we dont need to wrap like in with case
    [(Bind bound-ids bound-bodies brang) (sub (Call (Fun bound-ids brang) bound-bodies))]
    [(Bind* bound-ids bound-bodies brang) (if (= 0 (length bound-ids))
                                              (sub brang)
                                              (sub (With (first bound-ids) (first bound-bodies)
                                                    (Bind* (rest bound-ids) (rest bound-bodies) brang))))])) ;; Needs Recursion
...
(test (run "{bind {{x 1} {y 2} {z 3}} z}") => 3)

(test (run "{bind* {{x 1} {x {+ x 1}} {x {* x 2}}} x}") => 4)
(test (run "{bind* {{x 1} {x {+ x 1}} {x {* x 2}} {y {* x x}}} y}")
      => 16)
...
;; TESTS FOR "Discussing the Previous Extension":

;; one of the bugs, you should not be able to pass duplicate argument
;; identifiers into a function
(test (run "{call {fun {x x} {+ {+ x x} 1}} 1 2}") => 5)

#|
i.e doing the same thing as above test in racket but throws error
...
;; should not be able to still use the fun keyword to make new functions
;; when it is defined as a parameter in the outer scope
(test (run "{call {fun {x fun} {+ fun {+ x {call {fun {y} y} 2}}}} 5 7}")
      => 14)
...
(: plus-two : Number Number -> Number)
(define (plus-two x define)
  (define (y z) z)
  (+ x (y 3)))
